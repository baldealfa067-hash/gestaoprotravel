...
BEGIN
  ...
  ELSIF NEW.tipo = 'pagamento_cliente' THEN
    ...
    IF NEW.bilhete_id IS NOT NULL THEN
      ...
      -- Descobrir o modo da companhia do bilhete
    ...
    -- Aplicar parte do custo:
    -- Companhia intermediária (credito): devolve ao saldo da companhia (abate dívida a ela),
    -- NÃO entra no Capital Circulante.
    -- Companhia com saldo próprio (recarregada): entra no Capital Circulante (conta destino).
    IF v_credito_conta > 0 THEN
      IF v_bilhete_modo = 'credito' AND v_bilhete_companhia IS NOT NULL THEN
        UPDATE public.companhias_aereas
          SET saldo = saldo + v_credito_conta, updated_at = now()
          WHERE id = v_bilhete_companhia;
      ELSIF NEW.conta_destino_id IS NOT NULL THEN
